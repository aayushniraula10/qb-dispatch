...
local function yazarkasa(camId)
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "yazarkasa", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-20",
        firstStreet = locationInfo,
        gender = gender,
        camId = camId,
        ...
        dispatchMessage = "Cash Register Robbery", -- message
        job = {"police"} -- jobs that will get the alerts
    })
end exports('yazarkasa', yazarkasa)



local function sivildown(camId)
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "sivildown", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-13A",
        firstStreet = locationInfo,
        gender = gender,
        camId = camId,
...
end exports('sivildown', sivildown)



local function casino(camId)
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "casino", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-80",
        firstStreet = locationInfo,
        gender = gender,
        camId = camId,
        ...
        dispatchMessage = "Casino Robbery", -- message
        job = {"police"} -- jobs that will get the alerts
    })
end exports('casino', casino)
